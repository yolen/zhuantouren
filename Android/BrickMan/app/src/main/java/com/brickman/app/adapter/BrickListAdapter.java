...
public class BrickListAdapter extends BaseQuickAdapter<BrickBean> {
    ...
    protected void convert(BaseViewHolder helper, BrickBean item) {
        Glide.with(mCtx).load(item.id).centerCrop().crossFade().placeholder(R.mipmap.ic_launcher).into((CircleImageView)helper.getView(R.id.avator));
        helper.setText(R.id.name, item.userId);
        ...
        helper.setImageResource(R.id.iconComment, item.contentBricks > 0 ? R.mipmap.bm_comment_sel : R.mipmap.bm_comment_nor);
        helper.setText(R.id.commentNum, item.contentBricks + "");
        ...
        mImagesAdapter = new ImagesAdapter(mCtx, linearLayout, item.brickContentAttachmentList);
        mImagesAdapter.init();
    }
}
