 ...
 */
public class BrickDetailModel implements BrickDetailContract.Model {
    @Override
    public void loadDetail(int contentId, HttpListener httpListener) {
        RequestParam params = ParamBuilder.buildParam("contentId", contentId + "");
        RequestHelper.sendGETRequest(true, Api.GET_CONTENT_DETAIL, params, httpListener);
    }
    ...
    @Override
    public void updateShareCount(String contentId, HttpListener httpListener) {
        RequestParam param = ParamBuilder.buildParam("contentId", contentId);
        RequestHelper.sendGETRequest(false, Api.SHARE_UPDATE_COUNT, param, httpListener);
    }
}
